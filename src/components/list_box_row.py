...
class ListBoxRow(Gtk.ListBoxRow):
    ...
    def action_target_string(self) -> str | None:
        value = self.get_action_target_value()
        return "" if value is None else value.get_string()

    def get_action_target_string(self) -> str:
        return self.get_property("action_target_string")

    @action_target_string.setter
    def action_target_string_setter(self, value: str) -> None:
